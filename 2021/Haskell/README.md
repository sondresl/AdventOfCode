...
The task is to insert letters between pairs of letters to grow an initial
string into a very large string.

Very similar to day 8 (with the lanternfish), I keep track of the frequencies
in a map.  For every generation, each pair sets itself to zero, and add equal
to itself the two pairs generated by placing a new letter in between the
letters in itself. Then, when all the new pairs are gathered together into a new map 
it results in the next generation. Repeat.
...
It is possible to keep track of the number of letters as well, but I do not.
Instead, make a count based on the final tally of pairs. This can be done by
counting the first element in each pair, and then adding onto the total tally
whatever the final letter in the original polymer was, since this letter is
never double or removed.
...
This is a cool way to find the difference between the maximal and minimal
element in the list, using `(<*>)` as an S-combinator again.
...
The task today to find the cheapest way from the top left to the bottom right
through a grid of numbers. Part to increases the size of the board by five in 
both directions.

I originally used a `dijkstra` implementation from `Algorithms.Search`, but it
was pretty slow so I switched to doing it manually with a priority queue.

This is the shortest path function:
...
The function is pretty standard, but uses the `MultiWayIf` language extension,
which is pretty useful. Since I do not mutate the queue in Haskell, `deleteFindMin`
returns a tuple of the minimal element and the queue without that element.

The function is also not general, but instead returns the cost immediately upon
finding the target coordinate. Overall, it take over three seconds to do part
2, which is not optimal, but I am not sure how I can optimize further without
using mutation. There is probably overhead with allocation in the priority
queue since it is persisent.

The `findBounds` function was very useful today, as I used it twice.
...
[Code](src/Day16.hs) | [Text](https://adventofcode.com/2021/day/16)

What an absolute banger of a day. Today I found use for `Parsec` on a list of ints,
recursion schemes, _and_ plated lenses!

I. Parsing into `Packet`

The actual packet type looks like this:

```haskell
data Packet = Packet Int Int (Either Int [Packet])
```

So each `Packet` contains two ints (`versionId` and `typeId`), and then either
just an int (if it is a literal packet) or a list of packets (if it is an
operator).

First, the hex number is made into binary, as a list of ones and zeros. Then, I use
`Parsec` to parse this list of ints into packets.

```haskell
type Parser = P.Parsec [Int] ()
```

Normally one considers parsing as an operation on strings, or lists of
characters, but the method works on list of others things as well. In this
case, it is possible to parse one element from the list using `anyToken`. Using
this will return this element *and* move the parser one along in the list being
parsed. Thus, we can parse packets with the following function:
...
`ints` reads `n` elements from the list and parses that binary number into a
base-10 numbers, so the first lines does this twice and puts the results in a
tuple. Then, based on the `typeId` either a literal packet is parsed or an
operator packet is parsed. Both these functions and the packet function itself
returns both the returned value and the number of bits parsed since this is
necessary when parsing operators.

Of the different parsers, `operator` is recursive and will call the packet
parser either `n` times or until `n` bits have been parsed based on the
`typeId` for that operator.

Once the *parsing* is finished, a recursive packet type will be returned,
holding all the information about the input.

II. Plated Lenses

For part 1 we need to sum all the version numbers in this recursive structure.
We could write a recursive function on `Packet`s, but that is so imperative.
Instead, we use `lens` to *focus* every version number in the structure and ask
for the sum of this focus.

This is the line of code that does this. The keys function here is `cosmos`,
which can return every sub-element in the recursive structure. On a list it is
equivalent to the `tails` function, and in this case we get access to every
`Packet` in the recursive structure, including all the ones in the list that is
inside operator packets.

```haskell
sumOf (cosmos . _Packet . _1) tree
```

This *magic* is made possible by having the data type derive `Plated`
instances. These instances can be derived for recursively defined data
structures to automatically traverse the entire structure and return the values
at every level. It is also necessary to derive `Data`, and to make prisms such
as `_Packet` to *focus* inside of packets only.

Then inside every packet we focus on the first int which is the version number,
and all these numbers are summed to give us the answer to part 1.

III. Recursion Schemes

For the second part, the task asks us to reduce the entire tree down into a
single value, reducing parts differently depending on the `typeId`. Since the
data type itself is recursive, we do not have to specify *how* to recurse on
it. Instead, by deriving the *base functor* for `Packet`, it is possible to
define a function for a single level and abstract over recursion.
...
There are several things to notice here: First, this function works on
`PacketF`, even though we have not defined this type. This is the derived base
functor. Second, there is no actual recursion in this function.  Where there
would normally be recursion, inside the `Right`, there is instead an `Int`.
This is because this function works bottom up, and so the recursive elements
will already have had this function applied to them. The actual recursion is
done by the `cata` function, which knows how to apply this function over the
normal `Packet` data type.

Overall, a great task, and I will be surprised if there is a better one later
in the calendar.
...
[Code](src/Day17.hs) | [Text](https://adventofcode.com/2021/day/17)
...
[Code](src/Day18.hs) | [Text](https://adventofcode.com/2021/day/18)

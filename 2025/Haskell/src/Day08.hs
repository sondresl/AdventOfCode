...
import Linear (V3(..), qd, _x)
import Control.Lens (view)
import Data.Ord (Down(Down))

type JunctionBox = V3 Int
...
solve boxes = go (Set.fromList $ map Set.singleton boxes) js
  where
    ...
    js :: [Set JunctionBox]
    js = map (\(x,y) -> Set.fromList [x,y]) . sortOn (uncurry qd) . map tuple $ combinations 2 boxes
...
main = do
  ...
  print $ product . Set.map (view _x) . fst <$> find ((== 1) . Set.size . snd) states

parseInput :: String -> [JunctionBox]
parseInput = map ((\[x,y,z] -> V3 x y z) . allNums) . lines
